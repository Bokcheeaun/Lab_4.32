package com.example.taruc.lab_43;
...
import com.example.taruc.lab_43.UserContract.User;
...
public class UserSQLHelper extends SQLiteOpenHelper{
    ...
    //Help you to link your activity to Database
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(SQL_CREATE_ENTRIES); //Create the table we need in the application
    }
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) { //oldVersion = Old field / newVersion = New field
    ...
    public void insertUser(UserRecord userRecord){
        ...
        //Insert a row
        SQLiteDatabase database = this.getWritableDatabase(); //Make changes to the database = INSERT/EDIT
        database.insert(User.TABLE_NAME, null, values); // null is for every field to be insert. If want specified field to be entered, put the field name
    ...
    //Get all records
    public List<UserRecord> getAllUsers(){ //Get all the data and return as a List
        List<UserRecord> records = new ArrayList<UserRecord>();

        //getReadableDatabase to retrieve/read records
        SQLiteDatabase database = this.getReadableDatabase(); //Get contents from the database
